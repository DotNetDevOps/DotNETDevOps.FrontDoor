import { Bar, mixins as test } from 'vue-chartjs';
import Vue from 'vue'
...
@Component
export default class BarChart extends mixins(Bar, test.reactiveProp) {

  ctx: CanvasRenderingContext2D | null = null;
  chartId: string | undefined ;

  @Prop({ default: ()=>[1, 0.4, 0], type: Array, validator: val=>val.length>2 }) readonly gradientStops !: [number, number, number];
  @Prop({ default: ()=>['rgba(72,72,176,0.2)', 'rgba(72,72,176,0.0)', 'rgba(119,52,169,0)'], type: Array, validator: val => val.length > 2 }) readonly gradientColors !: [string, string, string];
